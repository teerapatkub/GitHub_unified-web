...
5. **Arcade Items Database Table & Seeding**: สร้างตาราง `arcade_items` ใน PostgreSQL พร้อมเพิ่มข้อมูลไอเทมก่อกวน/บัฟเรียลไทม์ทั้ง 15 ชนิด (`item_code`, `name_th`, `name_en`, `desc_th`, `desc_en`, `price`, `icon`, `type`) และเชื่อมต่อ API `GET /api/arcade/items` ให้ดึงตรงจาก PostgreSQL
6. **Cleanup Temporary Log Files**: ทำการลบไฟล์บันทึกการทดสอบชั่วคราว (`server/*.log`) ทั้งหมดในไดเรกทอรี `server/` เพื่อประหยัดพื้นที่ โดยคงโฟลเดอร์ `prototype/` และ `client/src/pages/old plan/` ไว้ตามที่ผู้ใช้ต้องการ
7. **Production Build Verification**: ทดสอบสั่ง `npm run build` ผ่านสมบูรณ์แบบไร้ข้อผิดพลาด (`✓ built in 20.42s`)
