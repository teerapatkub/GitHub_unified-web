...
export default function MiNi_Game({ lessonId, user, onUserRefresh, onNavigate }) {
  ...
  const subtopics = useMemo(() => {
    ...
    // Each subtopic carries the problem's Thai and English text. Resolve the
    // language once here so every reader downstream - the mission panel, the AI
    // context, the branch copy fallback - gets the same one.
    const localize = (subtopic) => ({
      ...subtopic,
      title: problemTitle(lang, subtopic),
      hint: problemDescription(lang, { description: subtopic.hint, description_en: subtopic.hint_en }),
    });

    const normalizedList = list.map(localize).map((subtopic) => {
